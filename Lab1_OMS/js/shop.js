$(document).ready(function() {
	...
	function addRow() {
		...
		/*	document.querySelector('.cart-body').insertAdjacentHTML(
			...
			$('.product0').val("Intel Core i5 12400F");
			$('.amount0').val(cart[i]);
					break;
			...
			$('.product1').val("Palit NVIDIA GeForce RTX 2060SUPER");
			$('.amount1').val(cart[i]);
					break;
			...
			$('.product2').val("Digma DGMAS31600008D");
			$('.amount2').val(cart[i]);
					break;
			...
			$('.product3').val("MSI B550 GAMING GEN3");
			$('.amount3').val(cart[i]);
					break;
